...
from .backtest import run_backtest, run_simple_backtest
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        description="AI Trading Platform - Experiment #001"
    )

    parser.add_argument(
        "command",
        choices=["backtest", "download"],
    )
    ...
    parser.add_argument(
        "--output-dir",
        default="data/processed/backtest_results",
...
def run_backtest_command(args) -> None:
    ...
    if args.data:
        ...
        if not data_path.exists():
            raise SystemExit(
                f"Backtest data file does not exist: {data_path}"
            )
        ...
        if df.empty:
            raise SystemExit(
                f"Backtest data file is empty: {data_path}"
            )
        ...
        print("No --data supplied; using sample data.")
        print(
            "NOTE: sample data is for pipeline validation only; "
...
def run_download_command(
    ...
    if not all(
        ...
        parser.error(
            "download requires --symbol, --start, and --end"
        )
...
def main(argv=None):
    ...
    if args.command == "backtest":
        ...
        run_download_command(args, parser)
